...
export interface Porteiro { id: string; nome: string; cpf?: string; senha: string; isAdmin: boolean; isDev?: boolean; condoId?: string; }
...
export class DbService {
  ...
  // EVENT BUS DE RESTAURAÇÃO (Sincronização de Contadores)
  public onRestore$ = new Subject<BackupData>();
  ...
  private readonly MASTER_CARRIERS_DATASET = [
    // CORREIOS E SERVIÇOS
    "CORREIOS", "SEDEX", "SEDEX 10", "SEDEX 12", "SEDEX HOJE", "PAC", "PAC MINI", "E-SEDEX", "CARTA REGISTRADA",
    ...
    "KABUM", "PICHAU", "TERABYTE", "LEROY MERLIN", "TOK STOK",
    "MADEIRAMADEIRA", "CARREFOUR", "ATACADAO", "ASSAI",
    // REGIONAIS
    "EXPRESSO ARAXA", "EXPRESSO JUNDIAI", "EXPRESSO NORDESTE",
    ...
    "NORDESTAO LOGISTICA", "SOLISTICA", "GOLOG", "COMETA LOGISTICA",
    "CONECTA LOG", "JET ENTREGA", "LEVE LOGISTICA", "ENVIAAQUI",
    "FRETEBRAS", "ITAMARATI NORTE", "ARCO IRIS TRANSPORTES",
    "EXPRESSO MARINGA", "TRANS NB", "TRANS EXPRESSO SUL", "TRANS MAZZA",
    "EXATA LOGISTICA", "DIRECIONAL LOGISTICA"
  ];
  ...
  async processBackupData(data: BackupData): Promise<boolean> {
      ...
      // --- NOTIFICAÇÃO DE RESTAURAÇÃO PARA SINCRONIA DE CONTADORES ---
      this.onRestore$.next(data);
  ...
  private async initDatabase() {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      request.onupgradeneeded = (event: any) => {
        ...
        ['porteiros', 'moradores', 'encomendas', 'logs', 'config', 'internal_backups', 'sys_handles', 'inbox'].forEach(store => {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        tx.oncomplete = () => {
          ...
          this.appConfig.set({ id: 'main_config', ocrTemperature: 0.3, ocrPromptMode: 'BALANCED', autoCorrectionEnabled: true, googleClientId: '', scannerFPS: 15, activePlan: 'PENDENTE', condoId: crypto.randomUUID(), kioskMode: false });
          resolve();
        };
