...
export class AuthService {
  ...
  private readonly USAGE_KEY = 'simbiose_usage_count';
  ...
  constructor() {
    ...
    const savedUsage = localStorage.getItem(this.USAGE_KEY);
    if (savedUsage) {
        this.usageCount.set(parseInt(savedUsage, 10));
    }
    ...
    // --- SINCRONIA DE CONTADOR PÓS-RESTAURAÇÃO ---
    // Escuta o evento de restauração do banco e atualiza o contador de uso
    // para refletir o número exato de encomendas ativas/validadas restauradas.
    this.db.onRestore$.subscribe(() => {
        // Aguarda um ciclo para garantir que o signal 'encomendas' foi atualizado
        setTimeout(() => {
            const totalRestored = this.db.encomendas().length;
            this.usageCount.set(totalRestored);
            localStorage.setItem(this.USAGE_KEY, totalRestored.toString());
            console.log(`[AuthService] Contador sincronizado pós-restore: ${totalRestored}`);
        }, 100);
    });
  ...
  public getPlanLimit(): number {
      ...
      if (plan === 'START') return this.currentFreeLimit();
      if (plan === 'BASICO') return this.LIMIT_BASICO;
      if (plan === 'PRO') return this.LIMIT_PRO;
      if (plan === 'ENTERPRISE') return this.LIMIT_ENTERPRISE;
      if (plan === 'DEV_SUPREMO' || plan === 'PRO_INFINITY') return this.LIMIT_DEV;
      return this.currentFreeLimit();
  }
  ...
  public async reportUsage(amount: number = 1): Promise<{ ok: boolean; message?: string; code?: string }> {
      ...
      localStorage.setItem(this.USAGE_KEY, next.toString());
      return { ok: true };
  }
  ...
  async completeLogin(user: Porteiro, overridePlan?: string) {
      ...
      if (user.id === 'luis_resolve_vip' || user.id === 'rodrigo_simbiose_vip') {
          ...
          this.db.saveAppConfig({ 
              ...
              nomeCondominio: user.id === 'luis_resolve_vip' ? 'Solve Prestadora' : 'Protocolo Inteligente'
          });
      }
