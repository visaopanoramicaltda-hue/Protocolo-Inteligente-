...
export class AdminHubComponent implements OnInit, OnDestroy {
  ...
  menuItems = computed(() => {
    ...
    if (this.isAssistedMode()) {
        ...
        return allItems.filter(item => assistedMenuIds.includes(item.id));
    }
    return allItems;
  });
  ...
  closeSecurityModal() { this.showSecurityModal.set(false); }
  ...
  moradoresGroups = computed(() => {
      ...
      if (query) {
          list.forEach(m => {
              ...
              const unitMatch = m.bloco.toLowerCase().includes(query) || m.apto.toLowerCase().includes(query);
  ...
  async importContactFromDevice() {
      // Feature request: Button to import contacts
      ...
      if (contact) {
          // Preenche automaticamente o nome se estiver vazio ou se o usuário quiser
          if (!this.moradorModalName()) {
  ...
  async deleteMorador(id: string) {
      const currentUser = this.auth.currentUser();
      // PERMISSÃO DE EXCLUSÃO: Apenas Admins
      if (!currentUser?.isAdmin) {
          this.ui.show('Acesso Negado: Apenas Administradores podem excluir moradores.', 'ERROR');
          this.ui.playTone('ERROR');
          return;
      }
      if (confirm('Tem certeza que deseja remover este morador?')) {
          await this.db.deleteMorador(id);
          this.ui.show('Morador removido.', 'SUCCESS');
      }
  }
  
  downloadCsvTemplate() {
      const csvContent = "\uFEFFNOME,BLOCO,APTO,TELEFONE\nExemplo Silva,A,101,11999999999";
      const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.setAttribute("href", url);
      link.setAttribute("download", "modelo_importacao_moradores.csv");
  ...
  async generateReport() {
      ...
      try {
          ...
          if (type === 'ENCOMENDAS') {
              ...
              // 3. PORTEIRO FILTRAGEM (ATIVIDADES COMPLETAS)
              if (this.reportFilterPorteiro() !== 'TODOS') {
                  const pid = this.reportFilterPorteiro();
                  // Pega onde ele recebeu (Entrada) OU onde ele entregou (Saída)
