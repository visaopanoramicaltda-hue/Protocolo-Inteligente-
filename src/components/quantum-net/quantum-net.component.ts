...
export class QuantumNetComponent implements OnInit, OnDestroy {
  ...
  // LOGO MERCADO PAGO - Carregada via Arquivo Local
  // Isso reduz o tamanho do bundle do componente e centraliza o asset
  trustedUrl: string = 'assets/icons/mercado-pago.svg';
  ...
  ativarPlanoDefinitivo() {
      if (this.planoSelecionado() === 'EXCEDENTE_PRO') {
          const currentUsage = this.auth.usageCount();
          this.auth.usageCount.set(Math.max(0, currentUsage - 1000));
          localStorage.setItem('simbiose_usage_count', this.auth.usageCount().toString());
