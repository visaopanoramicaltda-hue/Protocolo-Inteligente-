...
export class DashboardComponent implements OnDestroy {
  ...
  currentPdfUrl = signal<string | null>(null); // Armazena URL do PDF gerado
  ...
  constructor() {
      ...
      effect(() => {
          ...
          if (isViewingGroup) { this.ui.isImageViewerOpen.set(true); } else if (!this.ui.currentFullscreenImage()) { this.ui.isImageViewerOpen.set(false); }
      }, { allowSignalWrites: true });
      effect(() => { this.ui.isSignatureMode.set(!!this.withdrawalItem()); }, { allowSignalWrites: true });
      effect(() => {
  ...
  filteredEncomendas = computed(() => {
      ...
      const sortFunction = (a: Encomenda, b: Encomenda) => {
          if (filter === 'PENDENTE') {
              const blocoA = (a.bloco || '').toString();
              const blocoB = (b.bloco || '').toString();
              const blocoDiff = blocoA.localeCompare(blocoB, undefined, { numeric: true, sensitivity: 'base' });
              if (blocoDiff !== 0) return blocoDiff;
              const aptoA = (a.apto || '').toString();
              const aptoB = (b.apto || '').toString();
              return aptoA.localeCompare(aptoB, undefined, { numeric: true, sensitivity: 'base' });
          } else {
  ...
  stats = computed(() => { 
      ...
      return { 
          ...
          delivered: list.filter(e => e.status === 'ENTREGUE' && new Date(e.dataSaida!).getTime() >= startOfDay).length,
          canceled: list.filter(e => e.status === 'CANCELADA').length
      }; 
  });
  
  // SHELVES UPDATED: Agrupa por bloco e filtra estritamente pela busca (apenas Bloco/Apto)
  shelves = computed<Shelf[]>(() => { 
      const pending = this.encomendas().filter(e => e.status === 'PENDENTE'); 
      const search = (this.searchQuery() || '').toLowerCase().trim();
      const shelfMap = new Map<string, Encomenda[]>(); 
      
      pending.forEach(item => { 
          let key = 'GERAL'; 
          if (item.bloco) key = `BLOCO ${item.bloco}`; 
          
          if (!shelfMap.has(key)) shelfMap.set(key, []); 
          shelfMap.get(key)!.push(item); 
      }); 
      
      const shelvesArray: Shelf[] = []; 
      
      shelfMap.forEach((items, title) => { 
          // FILTRAGEM PRATELEIRA: 
          // Se houver busca, exibe a prateleira APENAS se algum item nela corresponder ao termo de Bloco ou Apto.
          if (search) {
              const hasMatch = items.some(i => 
                  (i.bloco || '').toLowerCase().includes(search) || 
                  (i.apto || '').toLowerCase().includes(search)
              );
              if (!hasMatch) return;
          }
          shelvesArray.push({ title, items }); 
      }); 
      
      return shelvesArray.sort((a, b) => a.title.localeCompare(b.title)); 
  });

  isDatabaseEmpty = computed(() => this.db.moradores().length === 0);
  
  // UPDATED: Now supports autoStart parameter to trigger camera immediately
  handleScannerAccess(route: string, autoStart: boolean = false) { 
      if (!this.auth.hasActiveFeatureAccess()) { 
          this.ui.show('Funcionalidade bloqueada no plano atual.', 'WARNING'); 
          return; 
      } 
      
      // Navigate with optional query param for instant start
      this.router.navigate([route], { 
          queryParams: autoStart ? { autoStart: 'true' } : {} 
      }); 
  }
  ...
  // LEGACY: Abre modal (Mantido para compatibilidade, mas substituído na view principal pelo openShelfAsList)
  openShelf(shelf: Shelf) { this.selectedShelf.set(shelf); }
  
  // NEW: Ação de Drill-Down (Prateleira -> Lista Filtrada)
  openShelfAsList(shelf: Shelf) {
      // Extrai o nome do bloco (remove 'BLOCO ') para usar como filtro
      const cleanTitle = shelf.title.replace('BLOCO ', '').trim();
      
      // Configura a view para lista
      this.searchQuery.set(cleanTitle);
      this.activeFilter.set('PENDENTE');
      this.viewMode.set('LIST');
      
      // Dispara a busca e scroll
      this.ui.show(`Visualizando ${shelf.title}`, 'INFO');
      this.onInlineSearch();
  }
  ...
  // LOGIC FIX: Restringe a busca apenas aos moradores da unidade SE houver moradores na unidade.
  // Isso impede que nomes de outros apartamentos apareçam na lista.
  filteredResidents = computed(() => { 
      const term = (this.withdrawalReceiverName() || '').toUpperCase().trim(); 
      const contextResidents = this.unitResidentsSuggestions();

      // MODO ESTRITO: Se houver contexto de unidade (pacote com Bloco/Apto), busca APENAS nele.
      if (contextResidents.length > 0) {
          if (term.length === 0) return contextResidents;
          return contextResidents.filter(m => (m.nome || '').toUpperCase().includes(term));
      }

      // MODO FALLBACK: Se o pacote não tiver unidade, permite busca global (mas ainda requer digitação)
      if (term.length === 0) return [];
      return this.db.moradores().filter(m => (m.nome || '').toUpperCase().includes(term)).slice(0, 4); 
  });
  ...
  startDraw(e: any) { if (e.cancelable) e.preventDefault(); this.isDrawing = true; this.canvasCtx?.beginPath(); this.canvasCtx?.arc(this.getPos(e).x, this.getPos(e).y, 0.5, 0, 2 * Math.PI); this.canvasCtx?.fill(); this.lastPoint = this.getPos(e); }
  ...
  async confirmSignature() {
      ...
      try {
          ...
          if (porteiro) {
              ...
              this.currentPdfUrl.set(url); // STORE IT HERE
